...
public class State implements Comparable<State> {
    ...
    public void initializeChildren() {
        Move[] moves = board.getPossibleMoves(player);
        children = new State[moves.length];
        for(int i=0; i<children.length; i++) {
        	Board b = new Board(board);
        	b.makeMove(moves[i]);
        	//System.out.println("child of " + player + " is " + player.opponent());
        	children[i] = new State(player.opponent(), b, moves[i]);
        }
    }
...
//        System.out.println("State.toString printing");
    ...
    public @Override int compareTo(State st) {
        int me = getValue();
        int other = st.getValue();
        
        if(me < other) return -1;
        else if(me == other) return 0;
